...
├───data/  # 数据集
├───frontend/        # 前端代码
│   ├───home_page.py # streamlit 主页
│   ├───plot.py      # 绘图函数，可在主页中调用
│   ├───style.py     # 样式文件，存储标题样式，作为可调用的函数
│   └───__init__.py
├───img/
├───log/             # 日志文件
├───metadata/        # 元数据，存储 API 列表
│   └───api.list
├───package/               # 项目代码
│   ├───callbacks.py       # 提供一些类，可在训练过程中监控、记录和可视化模型的性能指标
│   ├───data_module.py     # 数据模块，用于加载数据集
│   ├───download_apks.py   # 下载 APK 文件数据集
│   ├───model.py           # 定义模型
│   ├───plot_callgraph.py  # 绘制调用图
│   ├───process_dataset.py # 处理数据集，提取调用图
│   ├───utils.py           # 工具函数
│   └───__init__.py
├───plot/
│   └───plot_f1_score.py    # 绘制 F1 Score 对比曲线
├───train_model.py          # 训练模型
├───predict.py              # 预测单个 APK 文件
├───main.py                 # 主程序入口，用于启动前端
├───generate_sha256_file.py # 生成 SHA256 文件
├───is-task-2.pdf
├───.gitignore
├───LICENSE
├───README.md
...
![procedure-zh](img/procedure-zh.svg)
...
2. GraphConv，添加 dropout 层与不添加 dropout 层的 F1 分数对比：

![GraphConv](img/GraphConv.svg)

3. SAGEConv，添加 dropout 层与不添加 dropout 层的 F1 分数对比：
